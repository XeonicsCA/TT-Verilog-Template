...
async def test_default_routing_and_handshake(dut):
    ...
    A1, A2, B1, B2 = 0x12, 0x34, 0x56, 0x78
    dut.a1.value = A1
    dut.a2.value = A2
    dut.b1.value = B1
    dut.b2.value = B2
    ...
    dut.op.value = 0x00  # NOOP localparam in decode module
...
async def test_opcode_changes_ctrl(dut):
    ...
    What this test verifies:
      ...
      - Change the opcode to a non-NOOP (e.g., VADD2 = 0x06 in your scheme).
      - The packed control bits should differ from the NOOP baseline.

    Expected outcome:
      - ctrl_flat differs between NOOP (0x00) and the non-NOOP opcode.
    """
    cocotb.start_soon(Clock(dut.clk, CLK_NS, units="ns").start())
    await reset(dut)
