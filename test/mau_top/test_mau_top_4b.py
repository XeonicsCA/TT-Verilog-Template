...
CLK_NS = 10           # 100 MHz system clock
SPI_CLK_CYCLES = 5    # usage requires 5 SPI clk cycles to load in instruction
...
# Reset the MAU top-level and initialise IOs
async def reset(dut):
    dut.rst_n.value = 0
    dut.ena.value   = 0
    dut.ui_in.value = 0
...
def alu_model_for_mau(op, a1, a2, b1, b2):
    ...
    # Fallback (shouldn’t be hit if opcodes are in range)
    return 0, 0


# Write one 4-bit nibble via RX SPI using ui_in[3:0] and uio_in[0]/[1].
# Mirrors test_rx_4b.py:
#  - Data on MOSI
#  - SPI clock low for a few cycles
#  - SPI clock high (sample) for a few cycles
#  - Return to low
async def spi_write_nibble(dut, nibble):
    ...
    # SPI clock high: RX samples MOSI here
    dut.uio_in[0].value = 1
    await ClockCycles(dut.clk, SPI_CLK_CYCLES)

    # SPI clock low again (end on low)
...
async def spi_send_instruction(dut, op, a1, a2, b1, b2):
    ...
    # Deassert write-enable and end with clock low
...
async def spi_read_nibble(dut):
    ...
    # SPI clock high: TX drives MISO (uo_out[3:0])
    dut.uio_in[0].value = 1
    ...
    # End on low so TX sees a falling edge
    dut.uio_in[0].value = 0
    await ClockCycles(dut.clk, 1)
...
async def mau_transaction(dut, op, a1, a2, b1, b2):
    ...
    # Generous but simple, like your other tests.
...
async def test_mau_dot2(dut):
    ...
    # Same flavour as ALU unit-test vectors
    vectors = [
        (2, 3, 1, 4),   # 2*3 + 1*4 = 10
        (5, 7, 3, 2),   # 5*7 + 3*2 = 41
        (9, 2, 4, 1),   # 9*2 + 4*1 = 22
    ...
    for op in ops_to_stress:
        for (a1, a2) in extreme_pairs:
            for (b1, b2) in extreme_pairs:
                ...
                # dut._log.info(msg)  # print output of every test to debug
...
# Sanity test for NOOP, DOT2, and SCMUL using randomly sampled operands
...
async def test_mau_noop_and_scalar_ops_sanity(dut):
    ...
    # nibble set contining a range of values
    vals = [0x0, 0x1, 0x3, 0x7, 0x8, 0xC, 0xF]

    random.seed(0x123ABC)
    max_samples_per_op = 32  # keep this reasonable so sim doesn't feel like it's hanging
